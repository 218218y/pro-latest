...
import { WALL_COLOR_EVENING } from './settings_visual_shared_contracts.js';
import { clamp, getLightBounds, LIGHT_PRESETS } from './settings_visual_shared_lighting.js';

export type SettingsVisualLightingController = {
...
export type CreateSettingsVisualLightingControllerArgs = {
  app: AppContainer;
...
function reportNonFatal(args: CreateSettingsVisualLightingControllerArgs, op: string, err: unknown): void {
  try {
...
export function buildSettingsVisualLightingPatch(app: AppContainer, on: boolean): UiPatch {
...
export function createSettingsVisualLightingController(
  args: CreateSettingsVisualLightingControllerArgs
): SettingsVisualLightingController {
  return {
    setLightingControl: (on: boolean) => {
      const patch = buildSettingsVisualLightingPatch(args.app, on);
      patchUiLightingState(
        args.app,
        patch,
        args.meta.uiOnlyImmediate('react:settingsVisual:lightingControl')
      );
    },
    setLightValue: (key: LightingScalarKey, value: number) => {
      ...
      setUiLightScalar(
        ...
        key,
        String(next),
        args.meta.uiOnlyImmediate('react:settingsVisual:lightSlider')
      );
    },
    applyLightPreset: (name: LightPresetName) => {
      ...
      patchUiLightingState(args.app, patch, args.meta.uiOnlyImmediate('react:settingsVisual:lightPreset'));
      ...
      try {
        ...
        reportNonFatal(args, 'settingsVisualLighting:applyLightPreset', err);
      }
    },
  };
}
