...
test('[stageI] semantic UI/config callsites preserve lighting + hot config writer seams', () => {
  assertMatchesAll(
    ...
    [
      ...
      /setUiLastSelectedWallColor\((?:app|args\.app), value, (?:meta|args\.meta)\.uiOnlyImmediate\('react:settingsVisual:wallColor'\)\)/,
      /patchUiLightingState\((?:app|args\.app), patch, (?:meta|args\.meta)\.uiOnlyImmediate\('react:settingsVisual:(?:lightingControl|lightPreset)'\)\)/,
    ],
    'semanticCallsitesBundle'
  );
